...
% Status: COMPLETE FIX - numeric citations with correct keys
%═══════════════════════════════════════════════════════════
...
Such cost uncertainty is pervasive in infrastructure planning; as Kouvelis and Yu \cite{KouvelisYu1997} observe, forecasted parameter values rarely match realised costs in practice, motivating robust decision-making approaches.
...
\Cref{thm:mm-k2-hard} establishes weak $\mathsf{NP}$-hardness of min-max spanning trees with two discrete scenarios via a reduction from the \textsc{Partition} problem, constructing a grid graph where scenario costs encode target subset sums.
...
\Cref{app:notation} provides an alphabetised notation table for quick reference.```
