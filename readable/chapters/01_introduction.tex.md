...
These extensions represent active research directions and are briefly discussed in the outlook (\Cref{ch:conclusion}), but they lie outside the core scope of this work.
...
Fifth, we synthesise results into a comprehensive classification table (\Cref{ch:synthesis}).
\Cref{tab:complexity-landscape} organises eight problem variants (two objectives, two uncertainty models, two scenario count regimes) with their complexity classes and approximation guarantees.
...
\Cref{ch:synthesis} synthesises results from \Cref{ch:minmax,ch:regret} into a comprehensive classification table spanning eight problem variants.
...
\Cref{ch:conclusion} summarises achievements, acknowledges deliberate scope limitations, and surveys related robust optimisation models (budgeted uncertainty, two-stage formulations, recoverable robustness) along with open questions from the literature.
