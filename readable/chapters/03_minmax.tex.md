...
\begin{proof}
Every $c \in \Scenarios^{\Gamma}$ has the form $c_e = \bar{c}_e + \hat{c}_e \delta_e$ with $\delta \in [0,1]^{|E|}$ and $\sum_{e \in E} \delta_e \leq \Gamma$ (\Cref{def:budgeted-uncertainty}).
...
\wc{T} \;=\; \sum_{e \in E(T)} \bar{c}_e \;+\; \max\Biggl\{ \sum_{e \in E(T)} \hat{c}_e \delta_e \;:\; \delta \in [0,1]^{|E|},\ \sum_{e \in E} \delta_e \leq \Gamma \Biggr\}.
\end{equation}
...
The lemma assumes an integer budget, whereas \Cref{def:budgeted-uncertainty} allows any $\Gamma \in [0, |E|]$.
...
\Cref{def:budgeted-uncertainty} reaches that box at $\Gamma = |E|$, but against spanning trees the interval behaviour sets in earlier, at $\Gamma = n-1$: a tree offers the adversary only $n-1$ edges to spend on, and any further budget is wasted.
...
$T_1$ is therefore the cheapest tree when no deviation can occur, and among the most exposed as soon as one can occur; the optimum switches to $T_2$ at $\Gamma = 1$ and stays there.
...
Saturation arrives at $\Gamma = n-1 = 3$, well before the $\Gamma = |E| = 5$ at which \Cref{def:budgeted-uncertainty} reaches the full box, because a spanning tree of this graph offers only three edges on which to spend.
