...
The entries of $u$ are not all distinct, $e_3$ and $e_5$ sharing the upper bound $7$, so the sufficient condition of \Cref{rem:cost-ties} does not apply here; nevertheless the minimum is attained by $T_2$ alone, the seven remaining spanning trees having $u$-costs between $16$ and $20$, so $T_2$ is the unique min-max spanning tree, its cost matching the value $\wc{T_2} = 15$ found above.
