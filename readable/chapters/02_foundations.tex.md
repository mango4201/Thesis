...
\Cref{sec:uncertainty} then formalises the two uncertainty models, discrete scenarios and interval uncertainty, together with the min-max and min-max regret objectives.
...
The second is the cross-product of uncertainty models (discrete scenarios and interval uncertainty) with robust objectives (min-max cost and min-max regret), which yields the four problem variants analysed in \Cref{ch:minmax,ch:regret}.
