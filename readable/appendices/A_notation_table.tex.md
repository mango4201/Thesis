...
% Status: COMPLETE through Chapter 3 as locked (§3.1 to §3.3).
%   Ch3 introduces no thesis-wide symbol beyond wc(T) and the
%   upper-bound vector u: the reduction-local symbols (v_{i,j}, a_j,
%   b_j, r_j, T_P, W, Q, w(P) in §3.3.1; a_i, b_i, v_{i,j}, L_{i,j},
%   p in §3.3.2) are defined at the point of use and stay out of this
%   table by design.
%   Rows for §3.4 (Pi, pi, [x]^+) are prepared but commented out;
%   uncomment them when §3.4 is drafted.
%
...
% Uncomment with §3.4:
% $\pi$ & Price per unit of deviation budget (budgeted reformulation) & \S\ref{sec:mm-budgeted} \\
% $\Pi$ & Candidate prices $\{0\} \cup \{\hat{c}_e : e \in E\}$ & \S\ref{sec:mm-budgeted} \\
% $[x]^{+}$ & Positive part, $\max\{x, 0\}$ & \S\ref{sec:mm-budgeted} \\
\midrule
